...
total_cost=[x+1 for x in doubling_cost]

print('Doubling Cost\t Iteration\t Total Cost\t Potential\t Amortized Cost')
print(f"{doubling_cost[0]}\t\t{1}\t\t{total_cost[0]}\t\t{potential[0]}\t\t {total_cost[0]+potential[0]}")

for j in range(1,10):
    amortized_cost=total_cost[j]+potential[j]-potential[j-1]
    print(f"{doubling_cost[j]}\t\t{1}\t\t{total_cost[j]}\t\t{potential[j]}\t\t {amortized_cost}")
